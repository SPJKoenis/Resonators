...
class Resonator:
	...
	def __init__(self, x1, y1, frequency, w, s, TLi, TLv, TLg, DE, l_vert = 0):
		...
		self.y1 = y1
		self.l_vert = l_vert
		self.l_coupl = self.length/9 #the ratio between the coupling length and the total resonator length is constant
		print('coupling length:', self.l_coupl)
		self.r_outer = (self.d1*1.8-self.d)/2 +self.d  
	...
	def Waveguide(self, x1, y1, length, d_given):#, x_e, y_e):
		...
		r_inner = (self.d1*1.8-self.d)/2  #Difference between the two should be self.d
		...
		if l_horiz > width:
			...
			l_horiz = width
		rect = gdspy.Rectangle((x1 + r_outer, y1 - delta), (x1 + r_outer + self.l_coupl + delta/2.5, y1 + self.d + delta))
		...
		if delta == 0:
			result = gdspy.boolean(result, gdspy.Rectangle((x1 + r_outer + self.l_coupl, y1), (x1 + r_outer + self.l_coupl + (self.d1-self.d)/5, y1 + self.d)), 'or') #this defines the resonator end closest to the TL
