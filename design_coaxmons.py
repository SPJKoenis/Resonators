...
class Pads:
	...
	def Contact_Pad(self, x, y, d, d1, d2):
		r1 = gdspy.Polygon([(x, y), (x + d2, y), (x + d2 + 220, y - 160), (x + d2 + 220, y - 600),
							(x - 220, y - 600), (x - 220, y - 160)])
		self.restricted_area.append(r1)
		delta = 0.5*(d2 - d1)
		r2 = gdspy.Polygon([(x + delta, y), (x + d1 + delta, y), (x + d1 + delta + 220, y - 160),
							(x + d1 + delta + 220, y - 600 + delta),
							(x - 220 + delta, y - 600 + delta), (x - 220 + delta, y - 160)])
		delta = 0.5*(d2 - d)
		r3 = gdspy.Polygon([(x + delta, y), (x + d + delta, y), (x + d + delta + 120, y - 160),
							(x + d + delta + 120, y - 500 + delta),
							(x - 120 + delta, y - 500 + delta), (x - 120 + delta, y - 160)])
...
class Transmission_Line:
	...
	def __init__(self, pads, start_pad, finish_pad, d, d1, d2, layer):
		...
		self.d2 = d2
		self.layer = layer
		self.middle = coordinates(pads.reference_points[start_pad].x+2*d2, 2500)

	def GenerateTL(self, d, coords, rots, start = 'horizontal', right = False):
		delta = (d - self.d)/2
		coords = [coordinates(self.start.x + self.d2, self.start.y), coordinates(self.start.x + self.d2, 2500- self.d/2),
				  coordinates(self.end.x - self.d2, 2500 - self.d/2), coordinates(self.end.x - self.d2, self.end.y)]
		coords = [self.start] + coords
		coords.append(self.end)

		rots = [rotations(np.pi/2, 0), rotations(np.pi, 3*np.pi/2), rotations(-np.pi/2, 0), rotations(np.pi, np.pi/2)]

		return Line_With_Turns(coords, rots, self.d, d, start = start, right = right)
		#return result